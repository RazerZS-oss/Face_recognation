...
def test_model(test_encodings, test_names):
    correct_predictions = 0
    total_predictions = len(test_encodings)
    
    for test_encoding, test_name in zip(test_encodings, test_names):
        ...
        print(f"Predicted: {predicted_name}, Actual: {test_name}")
    
    accuracy = (correct_predictions / total_predictions) * 100
    print(f"Akurasi: {accuracy:.2f}%")

# Menguji model dengan data test
test_model(test_encodings, test_names)
